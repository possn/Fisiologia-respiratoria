...
# ============================================================
PPL_EE = -5.0            # Pleural at FRC
PPL_EI = -7.0            # Pleural at end inspiration (more negative)
...
def pmus_of_tau(tau: float) -> float:
    ...
    0 em CRF; sobe durante inspiração; mantém no fim-insp; cai na expiração.
    """
...
# ============================================================
# Precompute one-cycle signals with RC model
# ============================================================
N_PRE = 4000
...
V = np.zeros_like(tau_grid)        # volume above FRC (L)
...
# Scale volume to VT_TARGET for visuals
...
def ppl_of_tau(tau: float) -> float:    return interp_cycle(Ppl_scaled, tau)
def palv_of_tau(tau: float) -> float:   return interp_cycle(Palv_scaled, tau)
def pl_of_tau(tau: float) -> float:     return interp_cycle(PL_scaled, tau)
def flow_of_tau(tau: float) -> float:   return interp_cycle(Flow_scaled, tau)
def volume_of_tau(tau: float) -> float: return interp_cycle(V_scaled, tau)

# ============================================================
# Drawing: anatomical lungs + vessels (sem “bola branca”)
# ============================================================
def _mix(c1, c2, a):
...
# ============================================================
# Mini alveolus (reposicionado + pequeno + sem colisões)
# ============================================================
def draw_alveolus(ax, center=(0.15, 0.39), inflate=0.0):
    cx, cy = center
    w = 0.14 * (0.92 + 0.18*inflate)
    h = 0.16 * (0.88 + 0.20*inflate)
    edge = "#7c3aed"
    fill = (0.93, 0.90, 0.99)
    ...
    # label acima do alvéolo, com fundo branco (não tapa / não é tapado)
    ax.text(
        cx - 0.07, cy + 0.12, "Mini-alvéolo",
        fontsize=9.0, weight="bold", color=edge,
        bbox=dict(boxstyle="round,pad=0.10", facecolor="white", edgecolor="none", alpha=0.85),
        clip_on=True, zorder=85
    )

# ============================================================
# PL Thermometer
# ============================================================
def draw_pl_thermometer(ax, x=0.86, y=0.33, w=0.10, h=0.50, pl_value=6.0):
    pl_min, pl_max = 4.0, 10.0
    plv = float(np.clip(pl_value, pl_min, pl_max))
    frac = (plv - pl_min) / (pl_max - pl_min)

    ax.add_patch(Rectangle((x, y), w, h, fill=False, lw=2.0, edgecolor="#111827", alpha=0.85, zorder=20))
    ...
    ax.add_patch(Rectangle((x, y), w, y_low_top - y, facecolor="#e5e7eb", edgecolor="none", alpha=0.95, zorder=10))
    ax.add_patch(Rectangle((x, y_low_top), w, y_phys_top - y_low_top, facecolor="#bbf7d0", edgecolor="none", alpha=0.95, zorder=10))
    ax.add_patch(Rectangle((x, y_phys_top), w, y + h - y_phys_top, facecolor="#fde68a", edgecolor="none", alpha=0.95, zorder=10))

    ax.add_patch(Rectangle((x, y), w, h*frac, facecolor="#7c3aed", edgecolor="none", alpha=0.55, zorder=15))
    ax.plot([x-0.02, x+w+0.02], [y + h*frac, y + h*frac], color="#111827", lw=2.0, clip_on=True, zorder=25)

    ax.text(x + w/2, y + h + 0.02, "PL", ha="center", fontsize=10, weight="bold", color="#111827", zorder=30)
    ax.text(x + w/2, y - 0.045, f"{pl_value:.1f}", ha="center", fontsize=10, weight="bold", color="#7c3aed", zorder=30)

# ============================================================
# Semáforo Palv vs Patm (com zorder alto; sem caixa “escondida”)
# ============================================================
def gradient_semaphore(ax, palv_value, base_y=0.205):
    tol = 0.10
    if palv_value < -tol:
    ...
    ax.text(
        0.04, base_y + 0.06, "Semáforo do gradiente:",
        fontsize=10, weight="bold", color="#111827",
        clip_on=True, zorder=80
    )
...
for i in range(total_frames):
    ...
    # =========================
    # (D) Painel direito (sem sobreposições / sem texto cortado)
    # =========================
    ax_txt.set_xlim(0, 1)
    ax_txt.set_ylim(0, 1)
    ax_txt.axis("off")
    ...
    ax_txt.text(0.04, 0.95, "Leituras (agora)", fontsize=13.5, weight="bold",
                color="#111827", clip_on=True, zorder=90)
    ...
    ax_txt.text(0.04, 0.82, f"Palv = {palv_now:.1f} cmH₂O", fontsize=12.2, weight="bold",
                color="#2563eb", clip_on=True, zorder=90)
    ax_txt.text(0.04, 0.76, f"PL   = {pl_now:.1f} cmH₂O", fontsize=12.2, weight="bold",
                color="#7c3aed", clip_on=True, zorder=90)
    ...
    ax_txt.text(0.04, 0.70, f"Fluxo = {flow_now*60:.0f} L/min", fontsize=12.2, weight="bold",
                ...
                clip_on=True, zorder=90)

    ax_txt.text(0.04, 0.64, f"VT ≈ {vol_now*1000:.0f} mL", fontsize=12.2, weight="bold",
                color="#b91c1c", clip_on=True, zorder=90)

    # Fórmula: duas linhas, largura segura, nunca cortada
    ax_txt.text(
        0.04, 0.60,
        "Pressão transpulmonar:\nPL = Palv − Ppl",
        fontsize=11.2, weight="bold", color="#6d28d9",
        bbox=dict(boxstyle="round,pad=0.26", facecolor="#ede9fe", edgecolor="#6d28d9", alpha=0.98),
        clip_on=True, zorder=95
    )

    # Mini-alvéolo: abaixo da fórmula e acima do semáforo (sem colisões)
    draw_alveolus(ax_txt, center=(0.16, 0.39), inflate=inflate)

    # Termómetro: à direita
    draw_pl_thermometer(ax_txt, x=0.86, y=0.33, w=0.10, h=0.50, pl_value=pl_now)

    # Semáforo com zorder alto e fora do passo-a-passo
    gradient_semaphore(ax_txt, palv_now, base_y=0.205)

    # Passo-a-passo (sem “humor elástico”; mensagem clínica clara)
    ax_txt.text(
        0.04, 0.01,
        "Passo-a-passo (modelo R–C):\n"
        "Pmus↑ → Ppl↓ → Palv<0 → fluxo entra\n"
        "À medida que V↑, (V/C)↑ → ΔP↓ → fluxo desacelera → 0\n"
    ...
    # Evita cortes do suptitle e mantém espaçamentos estáveis
