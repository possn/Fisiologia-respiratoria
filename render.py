...
# OUTPUT
# ============================================================
OUT = "Fisiologia_Respiratoria_2_AulaBasica_Master_v5_FluxoFisio_LayoutOK.mp4"
...
# Flow peaks (L/s) -> só escala do pico
FLOW_PEAK_INSP = 0.45
FLOW_PEAK_EXP  = -0.35
...
def palv_of_tau(tau: float) -> float:
    """
    Curva fisiológica simples: 0 -> min -> 0 na inspiração,
    e 0 -> max -> 0 na expiração.
    """
    ph, x = phase_in_cycle(tau)
...
def _beta_bump(x: float, a: float, b: float) -> float:
    """
    Perfil 0->pico->0, normalizado (máximo=1).
    Pico em x* = a/(a+b).
    """
    ...
    y = (x**a) * ((1.0 - x)**b)
    xstar = a / (a + b)
    ystar = (xstar**a) * ((1.0 - xstar)**b)
    return float(y / ystar) if ystar > 0 else 0.0

def flow_of_tau(tau: float) -> float:
    """
    ✅ Fluxo fisiológico (espontâneo):
    acelera no início (gradiente maior) e desacelera até 0 (Palv→0).
    Sem degraus.
    """
    ph, x = phase_in_cycle(tau)
    if ph in ("hold_ee", "hold_ee2", "hold_ei"):
        return 0.0
    if ph == "insp":
        shape = _beta_bump(x, a=1.25, b=3.75)  # pico cedo
        return FLOW_PEAK_INSP * shape
    shape = _beta_bump(x, a=1.10, b=3.10)      # exp: pico cedo
    return FLOW_PEAK_EXP * shape
...
def draw_lungs_anatomic(ax, center=(0.43, 0.64), scale=1.0, inflate=0.0):
    ...
    rv_b1 = [(cx + 0.08*s*vs, cy + 0.07*s),
             (cx + 0.16*s*vs, cy + 0.07*s),
             (cx + 0.18*s*vs, cy + 0.03*s)]
    rv_b2 = [(cx + 0.13*s*vs, cy + 0.02*s),
             (cx + 0.20*s*vs, cy + 0.00*s),
             (cx + 0.20*s*vs, cy - 0.04*s)]

    lv_base = [(cx - 0.02*s, cy + 0.10*s),
               (cx - 0.08*s*vs, cy + 0.07*s),
               (cx - 0.13*s*vs, cy + 0.02*s),
               (cx - 0.14*s*vs, cy - 0.05*s)]
    lv_b1 = [(cx - 0.08*s*vs, cy + 0.07*s),
             (cx - 0.16*s*vs, cy + 0.07*s),
             (cx - 0.18*s*vs, cy + 0.03*s)]
    lv_b2 = [(cx - 0.13*s*vs, cy + 0.02*s),
             (cx - 0.20*s*vs, cy + 0.00*s),
             (cx - 0.20*s*vs, cy - 0.04*s)]

    for pts in [rv_base, rv_b1, rv_b2, lv_base, lv_b1, lv_b2]:
...
# ============================================================
# Mini alveolus (smaller + lower to avoid collisions)
# ============================================================
def draw_alveolus(ax, origin=(0.06, 0.39), inflate=0.0):
    ox, oy = origin
    w = 0.16 * (0.85 + 0.30*inflate)
    h = 0.16 * (0.60 + 0.55*inflate)
    edge = "#7c3aed"
    fill = (0.93, 0.90, 0.99)
    e = Ellipse((ox + 0.085, oy), width=w, height=h,
                facecolor=fill, edgecolor=edge, linewidth=2.6, alpha=0.95)
    ax.add_patch(e); e.set_clip_on(True)
    ax.text(ox, oy + 0.13, "Mini-alvéolo", fontsize=9.3, weight="bold", color=edge, clip_on=True)
...
def draw_pl_thermometer(ax, x=0.84, y=0.34, w=0.10, h=0.48, pl_value=6.0):
    ...
    ax.text(x + w + 0.03, y + 0.01, "baixo", fontsize=9, color="#6b7280", va="bottom", clip_on=True)
    ax.text(x + w + 0.03, y_low_top + 0.01, "ok", fontsize=9, color="#16a34a", va="bottom", clip_on=True)
    ax.text(x + w + 0.03, y_phys_top + 0.01, "alto", fontsize=9, color="#b45309", va="bottom", clip_on=True)

# ============================================================
# Semáforo Palv vs Patm (below alveolus)
# ============================================================
...
for i in range(total_frames):
    ...
    # cycle boundaries (for colored spans)
    ...
    thorax.set_clip_on(True)

    # bring label slightly down, above line thickness
    ax_anim.text(0.43, 0.875, "Caixa torácica", ha="center", va="center",
                 fontsize=9.2, color="#111827",
                 bbox=dict(boxstyle="round,pad=0.12", facecolor="white", edgecolor="none", alpha=0.75),
                 clip_on=True)

    # inflation normalized by reference PL range (~5→7)
    ...
    # airflow arrow (text positioned away from the black trachea)
    arrow_mag = float(np.clip(abs(flow_now) / max(abs(FLOW_PEAK_INSP), 1e-6), 0.0, 1.0))
    if flow_now > 1e-6:
        ax_anim.annotate("", xy=(0.43, 0.64), xytext=(0.88, 0.64),
                         arrowprops=dict(arrowstyle="->", lw=3 + 4*arrow_mag, color="#dc2626"))
        ax_anim.text(0.90, 0.70, "Ar entra", fontsize=9, color="#dc2626", ha="center", clip_on=True)
    elif flow_now < -1e-6:
        ax_anim.annotate("", xy=(0.88, 0.64), xytext=(0.43, 0.64),
                         arrowprops=dict(arrowstyle="->", lw=3 + 4*arrow_mag, color="#16a34a"))
        ax_anim.text(0.90, 0.70, "Ar sai", fontsize=9, color="#16a34a", ha="center", clip_on=True)
    else:
        ax_anim.text(0.90, 0.69, "Fluxo = 0", fontsize=9, color="#6b7280", ha="center", clip_on=True)
    ...
    ax_anim.text(0.82, dia_y + 0.02, "Diafragma", fontsize=9, color="#111827",
                 va="center",
                 bbox=dict(boxstyle="round,pad=0.12", facecolor="white", edgecolor="none", alpha=0.75),
                 clip_on=True)
    ...
    ax_p.plot(th, ppl_h,  lw=3.2, color="#111827", label="Ppl")
    ax_p.plot(th, palv_h, lw=3.2, color="#2563eb", label="Palv")
    ax_p.plot(th, pl_h,   lw=3.2, color="#7c3aed", label="PL")
    ...
    ax_p.scatter([t], [ppl_now], s=55, color="#111827", zorder=5)
    ax_p.scatter([t], [palv_now], s=55, color="#2563eb", zorder=5)
    ax_p.scatter([t], [pl_now], s=55, color="#7c3aed", zorder=5)
    ...
    # =========================
    # (D) Right panel: rigid bands (no overlap)
    # =========================
    ax_txt.set_xlim(0, 1)
    ax_txt.set_ylim(0, 1)
    ax_txt.axis("off")

    # band positions (tuned)
    Y_HEAD = 0.955
    Y1 = 0.885
    DY = 0.060

    ax_txt.text(0.04, Y_HEAD, "Leituras (agora)", fontsize=13.5, weight="bold",
                color="#111827", clip_on=True)

    ax_txt.text(0.04, Y1,          f"Ppl  = {ppl_now:.1f} cmH₂O", fontsize=12.2, weight="bold",
                color="#111827", clip_on=True)
    ax_txt.text(0.04, Y1 - 1*DY,   f"Palv = {palv_now:.1f} cmH₂O", fontsize=12.2, weight="bold",
                color="#2563eb", clip_on=True)
    ax_txt.text(0.04, Y1 - 2*DY,   f"PL   = {pl_now:.1f} cmH₂O", fontsize=12.2, weight="bold",
                color="#7c3aed", clip_on=True)

    ax_txt.text(0.04, Y1 - 3*DY,   f"Fluxo = {flow_now*60:.0f} L/min", fontsize=12.2, weight="bold",
                color=("#dc2626" if flow_now > 0 else "#16a34a" if flow_now < 0 else "#6b7280"),
                clip_on=True)

    # VT always visible (avoid formula collision)
    ax_txt.text(0.04, Y1 - 4*DY,   f"VT ≈ {vol_now*1000:.0f} mL", fontsize=12.2, weight="bold",
                color="#b91c1c", clip_on=True)

    # formula box: dedicated line BELOW VT (narrower + higher alpha)
    ax_txt.text(
        0.04, 0.61,
        ...
        weight="bold",
        color="#6d28d9",
        bbox=dict(boxstyle="round,pad=0.28", facecolor="#ede9fe", edgecolor="#6d28d9", alpha=0.98),
        clip_on=True
    )

    # alveolus placed lower; smaller (no overlap with box)
    draw_alveolus(ax_txt, origin=(0.06, 0.42), inflate=pl_norm)

    # thermometer right
    draw_pl_thermometer(ax_txt, x=0.84, y=0.34, w=0.10, h=0.48, pl_value=pl_now)

    # semaphore below alveolus
    ...
    ax_v.scatter([t], [vol_now], s=42, color="#2563eb", zorder=5)
