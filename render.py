...
# ============================================================
# R-C model parameters (reasonable teaching values)
# Units:
#   R in cmH2O / (L/s)
#   C in L / cmH2O
# ============================================================
R = 5.0       # typical-ish airway resistance
C = 0.10      # 100 mL/cmH2O (gives VT around 200 mL per 2 cmH2O)
# We'll scale VT visually later; physiologically this yields smooth waveforms.

# If you want bigger VT for visuals, increase C to 0.20.
# Keep it stable: R 4–8, C 0.08–0.25.
...
def pmus_of_tau(tau: float) -> float:
    """
    Pressure generated by inspiratory muscles.
    0 at FRC, ramps up during inspiration to PMUS_PEAK, holds, then ramps down in expiration.
    """
    ...
    if ph == "hold_ei":
        return PMUS_PEAK
    # expiration: muscle relaxes back to 0
...
# Alveolar pressure with airway opening at 0: Palv = -R*Flow
...
# Normalise V to a “didactic VT” of 0.5 L for visuals (keeps dynamics/shape)
VT_TARGET = 0.5  # L
...
# Recompute flow consistent with scaling (derivative)
Flow_scaled = np.gradient(V_scaled, dt)

# Recompute Palv for scaled flow (Palv = -R*Flow)
Palv_scaled = -R * Flow_scaled

# Keep Ppl from pmus (physiology), PL recomputed
Ppl_scaled = Ppl.copy()
...
# ============================================================
# Mini alveolus (small + positioned to avoid overlap)
# ============================================================
def draw_alveolus(ax, origin=(0.06, 0.40), inflate=0.0):
    ox, oy = origin
    w = 0.15 * (0.90 + 0.25*inflate)
    h = 0.15 * (0.75 + 0.35*inflate)
    edge = "#7c3aed"
    fill = (0.93, 0.90, 0.99)
    ...
    ax.text(
        ox, oy + 0.12, "Mini-alvéolo",
        fontsize=9.2, weight="bold", color=edge, zorder=50, clip_on=True,
        bbox=dict(boxstyle="round,pad=0.10", facecolor="white", alpha=0.80, edgecolor="none")
    )
...
for i in range(total_frames):
    ...
    # Label above line (with white bbox so it never hides)
    ax_anim.text(
    ...
    # diaphragm motion tied to Pmus (0..PMUS_PEAK)
    ...
    # =========================
    # (C) Fluxo (agora fisiológico: acelera e desacelera)
    # =========================
    ...
    draw_alveolus(ax_txt, origin=(0.06, 0.40), inflate=pl_norm)
    ...
    ax_txt.text(
        ...
        "Pmus↑ → Ppl↓ → V↑ → Palv<0 → fluxo entra\n"
        "V aumenta → V/C cresce → fluxo desacelera → 0\n"
        "Pmus↓ → recoil (V/C) domina → Palv>0 → ar sai\n\n"
        "Humor: o pulmão é um elástico com tubagem. Nada místico.",
        fontsize=8.6,
